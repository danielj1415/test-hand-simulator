...
const energyImageMapping: { [key: string]: string } = {
  ...
  const extractSetIdAndCardId = (cardName: string): [string, string] => {
    ...
    // If the card name contains extra descriptors, like "PH," ignore it
    // Ensure there are at least three parts to avoid errors
    if (parts.length < 3) {
      throw new Error(`Unexpected format for card name "${cardName}"`);
    }
  
    // Extract the set code and card ID as the last two relevant parts
    const setCode = parts[parts.length - 2];
    let cardId = parts[parts.length - 1];
  
    // Map the set code to its corresponding ID
    const mappedSetId = setCodeMapping[setCode];
    if (!mappedSetId) {
      console.warn(`Set code "${setCode}" not found in mapping for card "${cardName}". Skipping this card.`);
      return ["", ""]; // Return an empty string if the set code isn't recognized
    }
  
    // Pad the card ID with leading zeros if it's a Scarlet Violet or Sword & Shield card
