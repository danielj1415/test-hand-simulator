...
function App() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const shuffledDeck = rifleShuffle(cardArray);
  ...
  // Function to simulate a rifle shuffle
const rifleShuffle = (deck: string[]): string[] => {
  ...
  while (left.length > 0 || right.length > 0) {
    // Randomly decide which half to pull from next
    if (Math.random() > 0.5 && left.length > 0) {
      shuffledDeck.push(left.shift() as string); // Ensure TypeScript knows it's a string
    } else if (right.length > 0) {
      shuffledDeck.push(right.shift() as string); // Ensure TypeScript knows it's a string
    }
  }
