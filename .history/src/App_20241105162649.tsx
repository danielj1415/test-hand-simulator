...
function App() {
  ...
  useEffect(() => {
    const fetchSets = async () => {
      const tcgdex = new TCGdex('en'); // Initialize the SDK properly

      try {
        // Use the SDK's fetch method to retrieve sets
        const setsData: any[] | undefined = await tcgdex.fetch('sets'); // Fetch all sets
        if (setsData) {
          setSets(setsData); // Update state only if data is defined
        } else {
          console.warn("No sets data found");
        }
      } catch (error) {
